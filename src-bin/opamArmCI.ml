...
module Builder = struct
  ...
  let packages_of_repo {Repo.user;repo} =
    ...
    | "jpdeplaix","opam-repository" -> ["lwt";"async";"mirage";"datakit"]
    | _ -> ["ocamlfind"]
  ...
  let repo_builder ~revdeps ~typ ~opam_version ?volume target =
    ...
    let opam_repo = Opam_docker.repo ~user:"jpdeplaix" ~repo:"opam-repository" ~branch:"master" in
  ...
  let run_phases typ target =
    ...
    |`Ref ["heads";"master"] ->
       ...
       let archives =
         ...
         | {Repo.repo="opam-repository"; user} when Datakit_github.User.name user = "jpdeplaix"-> [archive_v2]
         | _ -> [] in
       archives @ base_tests
    |`Ref _  -> []
  ...
  let tests = [
    Config.project ~id:"jpdeplaix/opam-repository" (run_phases `Full_repo);
  ]
end
